...
Function Generate-EditForm {
    ...
    $CheckButton.Add_Click({ $Global:SaveVariables = $True })
    ...
    If($SaveVariables){ 
            $num = 0
        Foreach ($Value in $ButtonTextList){
            $Config[$num].ButtonText = $ButtonTextList[$num].Text
            $Config[$num].ButtonValue = $ButtonValueList[$num].Text
            
            #buttonlist[$num] = $buttontextlist[$num].text

            $num++
        }

        $Config | Export-Csv -Path '.\PasterTool-temp.config' -NoTypeInformation
        Remove-Item -Path '.\PasterTool.config'
        Rename-Item -Path '.\PasterTool-temp.config' -NewName 'PasterTool.config'

        #Change the buttons in the form with $form.button1 = x
        #need to make button generation the same as textbox where it makes a list
        #iterate through the list and assign the variables on each item
